        ...
        if self.instance:
            try:
                ...
                if sla_timer:
                    ...
                    # If SLA timer has no working hours, try from ticket organization
                    if not working_hours and self.instance.ticket_organization:
                        working_hours = getattr(self.instance.ticket_organization, 'working_hours', None)
                    ...
                    if working_hours:
                        ...
                        # Convert to aware IST time
                        now_ist = timezone.now().astimezone(tz)

                        # Convert to naive IST time for comparison
                        current_time = now_ist.time().replace(tzinfo=None)

                        start = working_hours.start_time  # naive
                        end = working_hours.end_time      # naive
